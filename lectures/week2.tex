...
\begin{example}
  ...
  What pattern of \(f(t)\) do you observe as \(t\) gets closer to \(1\) from the left \hlattn{but is not allowed to be \(1\)}?
...
\begin{itemize}
  \item ``\(t\) approaches \(a\) from the \hlmain{left}'' means 
  \item ``\(t\) approaches \(a\) from the \hlmain{right}'' means 
\end{itemize}
