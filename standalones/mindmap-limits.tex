...
% Thirteen pages: the whole map, then twelve layers of it -- the two-sided
% limit branch, the first decision, the 0/0 case, the nonzero/0 case, the
% evaluation techniques, the continuity branch, the continuous-or-not
% decision, the continuity test, why substitution works, the discontinuous
% branch, where an infinite discontinuity comes from, and the two limit
% branches together.  Each page is trimmed to
...
% ===================================================================
%  Page 8 -- the answer to the continuity test: the root, `continuity
%  at a', and the two answers it can have.  The one-sided tests that sit
%  between them are page 9, and what each answer leads to is on the pages
%  around it.
%
%  `missing' holds the places of those two tests, so `continuous' and
%  `discontinuous' fan out at page 1's angles.
% ===================================================================
\newcommand{\continuityanswerlayer}{%
  \node [concept, minimum size=3.8cm, text width=3.2cm, font=\small] (root)
    {limit at a constant \(a\)\\[0.8ex]
     % The nested picture inherits the mindmap's node sizing, which flings
     % these two labels centimetres away; reset it here.
    ...
    child [concept color=gray!20,
      ...
      % their places so `discontinuous' still lands on its own bar.
      child [missing]
      child [missing]
...
\resizebox{2.732in}{!}{%
\begin{tikzpicture}[map levels]
  \continuityanswerlayer
\end{tikzpicture} 
}
\end{mmpage}

% ===================================================================
%  Page 9 -- the continuity test: the root, `continuity at a',
...
% ===================================================================
%  Page 10 -- why substitution works: `continuity at a' and what
...
% ===================================================================
%  Page 11 -- the discontinuous branch: `continuity at a', the
...
% ===================================================================
%  Page 12 -- where an infinite discontinuity comes from: the
%  discontinuous branch of page 11 again, with `try direct substitution'
%  and its nonzero/0 case beside it and the arc that joins the two.  It is
%  the one place the map's two decision trees meet on the failing side.
%
%  Two subtrees with different level distances share this picture, so each
%  is drawn in a scope of its own rather than the levels being set on the
%  picture.
% ===================================================================
\newcommand{\infinitecaselayer}{%
  \begin{scope}[continuity levels]
  % `conttest' is this layer's root, placed where page 1 puts it: the first
  % of the root's branches, 150 degrees out at the level-1 5.5cm.  The
  % styling is what `every child node' gives it there, spelled out because
  ...
  % The other tree's root, placed where page 1 puts it: the second of the
  % root's branches, 30 degrees out at the level-1 5.5cm.  `missing' holds
  % the places of Squeeze and 0/0, so nonzero/0 keeps page 1's angle;
  % `any number/nonzero', the last child, simply goes.
  \begin{scope}[technique levels]
    \node [concept color=gray!20, concept, minimum size=2.4cm, text width=2.2cm,
           align=center, font=\footnotesize] (dirsub) at (30:5.5cm)
      {try direct\\substitution\note{is the denominator \(0\)?}}
  ...
  % A circular arc, bowed away from the root so it threads the gap between
  % the root and its branches.  No heads, to match page 1.
  \begin{pgfonlayer}{background}
    \draw [concept connection, pink]
      (constzero.south) to [bend left=20] (infinite);
  \end{pgfonlayer}
...
% ===================================================================
%  Page 13 -- what a limit is and what to do with it: the root, the
