...
%%% page 32: sin restricted and arcsin side-by-side
\begin{tikzpicture}
  \begin{axis}[
    name=left,
    sincos,
    ...
    \addplot[very thick, domain={-pi/2}:{pi/2},
      mesh, shader=interp, point meta=y] {sin(x)};
    \curveends{-pi/2}{-1}{pi/2}{1}
  \end{axis}
  \begin{axis}[
    at={(left.east)}, anchor=west, xshift={3ex},
    arcsin,
    xmin={-pi/2}, xmax={ pi/2},
    ymin={-pi/2}, ymax={ pi/2},
    enlargelimits,
    ...
    \addplot[ultra thick, mesh, shader=interp, point meta=y] {asin(x)};
    \curveends{-1}{-pi/2}{1}{pi/2}
  \end{axis}
\end{tikzpicture}
\end{document}
