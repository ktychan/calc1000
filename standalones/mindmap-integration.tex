...
\begin{tikzpicture}[
  ...
  \node (root) {integration}
    ...
    child [concept color=attn!20] { 
      ...
      child { node {recognize derivatives and use the integration table}  }
  ...
  \node [concept color=white, annotation, below right] at (ftcone.east) {\footnotesize \(\frac{d}{dx} \int_{a}^{x} f(t) \;dt = f(x)\)};
