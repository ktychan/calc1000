...
There are three limits where a graphical approach is the best we can do (with Calc 1000 tools). 
...
\emph{Try} a change of variable when we encounter a problem that looks like a familiar problem.
...
\begin{exercise}
  Evaluate \(\lim_{x \to 0} \frac{\sin(-x/2)}{x} \). Does the same trick work for \(\lim_{x \to 0} \frac{\sin(x)}{-x/2}\)? Is it necessary?

  \blanklines{15}
\end{exercise}
\clearpage

Let's revisit Example~\ref{ex:limit-rationalization}.  Rationalization is one way to \enquote{get rid of} the root.  A change of variable also works.
\begin{exercise}
...
\begin{exercise}
  Evaluate \(\lim_{x \to 1} \frac{x^{1/2} - 1}{x - 1}\).
...
Exercise~\ref{ex:limit-infinity-plus-constant} looks similar to Examples~\ref{ex:limit-cancellation} and \ref{ex:limit-rationalization} but is much simpler.
