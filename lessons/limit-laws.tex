...
Reflection helps us find connections and expand our skill set. Take a look at Exercise~\ref{ex:limit-cube-root}. At a glance, the three techniques introduced on pages \pageref{ex:limit-cancellation} and \pageref{eg:limit-change-of-variable} do not seem useful for this exercise. However, a combination of them works.  Let's start by revisiting Example~\ref{ex:limit-rationalization}. 
...
\begin{enumerate}[wide]
  \item Use the Squeeze Theorem to evaluate \(\lim_{x \to 0^{-}} x \sin(x)\).
  ...
  \item The two one-sided limits should evaluate to the same value. Conclude that \(\lim_{x \to 0} x \sin(x) = \underline{\hspace{1cm}}\).
\end{enumerate}

Example~\ref{ex:limit-textbook-2.28} is a subtle variation on the theme of Examples~\ref{ex:limit-cancellation} and \ref{ex:limit-rationalization} but challenges our algebra skills. It is a good practice, but leave it until you have mastered all other skills. Hopefully, the hint at the bottom of the page helps.
