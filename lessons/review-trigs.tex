...
We need to remember trigonometric equations often, but not always, have more than one solution. \updatenote{"solutions" corrected to "solution".}
If a question does not state a restriction for \(\theta\), then we need to find as many solutions as possible.

Moreover, Example~\ref{ex:trig-problem-solving-1} and \ref{ex:trig-problem-solving-2} together demonstrate the technique of \hlmain{change of variable}, which is fairly useful in mathematics.
...
\begin{exercise}
  ...
  \begin{table}[H]
    ...
    \caption{Special values for trigonometric functions \updatenote{"function" corrected to "functions".}}
...
\begin{exercise}
  Label each graph with its corresponding trig function. Which ones are even functions? Which ones are odd functions? \updatenote{"their" corrected to "its".}
  \bigskip
