...
\begin{lesson}{Applied optimization problems}
  ...
  \begin{example} \label{ex:optimization-fence}
    ...
    (3) Use the given scenario and our sketch to figure out the \hlmain{relation} among our variables. 
    \blanklines{5}

    (4) Eliminate all but one independent variable, so that the objective variable is a single-variable function in the remaining independent variable. \hlinfo{Also figure out the domain of the resulting function using the given scenario.}
  ...
  \begin{example} \label{ex:optimization-swim}
    ...
    (2) Identify the \hlwarn{objective variable}. Write ``\hlattn{Goal: We want to maximize/minimize \ldots{}}''. 
    \blanklines{3}
    ...
    (5) \hlattn{Achieve the goal we wrote down in Step (2)}. Be very clear of the mathematical problem we are about to solve.
    \blanklines{15}
  \end{example}
  ...
  \begin{example} \label{ex:optimization-closest-point-to-curve}
    ...
    (5) \hlattn{Achieve the goal we wrote down in Step (2)}. Remember we have to find the associated \((x,y)\) values as well. 
