...
\begin{enumerate}[label=(\alph*)]
  ...
  \item However, it is often easier to \hlmain{describe} real-world problems using more than one variables.
...
\begin{example} \label{ex:optimization-fence}
  ...
  \begin{enumerate}[wide, noitemsep, label=(Step~\arabic*).]
    ...
    \item Eliminate all but one independent variable, so that the objective variable becomes a single-variable function in the remaining independent variable. \hlinfo{Also figure out the domain of the resulting function using the given scenario.}
...
\begin{example} \label{ex:optimization-swim}
  A  river is \(1\) km wide. A person is standing on the bank of the river and want to reach the opposite side, \(2\) km down the river. The person can swim \(3\) km/hr and walk \(6\) km/hr. 
