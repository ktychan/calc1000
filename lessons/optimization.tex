...
To optimize is to find the absolute maximum or absolute minimum of some function. The mathematical idea of modern machine learning (or AI) is to find a function (given some known data set) that \emph{minimizes} a certain type of error measurement. \updatenote{Added the missing "a".} The resulting function is used to \emph{predict} the future.
...
Here are the big ideas. \updatenote{"re" corrected to "are".}
\begin{enumerate}[label=(\alph*)]
  ...
  \item However, it is often easier to \hlmain{describe} real-world problems using more than one variable. \updatenote{"variables" corrected to "variable" ("more than one" takes a singular noun).}
...
\begin{enumerate}[wide, label=(Step~\arabic*).]
  \item Sketch the given scenario and \emph{introduce variables} to measure relevant quantities.
  ...
  \item Treat the objective variable as the \emph{dependent variable} and all other variables as independent. Eliminate all but one independent variable, so that the objective variable becomes a single-variable function in the remaining variable.

    Common misadventures at this step.
    \begin{itemize}
      ...
      \item Sometimes, especially when we are stressed or under time pressure, we eliminate the objective variable away. Step 2 ensures that we always keep the goal in sight.
    \end{itemize}
...
\begin{example} \label{ex:optimization-fence}
  We have \(100\) metres of fence material and want to enclose a rectangular area with one side against the wall. The interior is \emph{evenly} divided into two sub-areas by putting up a fence perpendicular to the wall. What is the largest area we can enclose?
  ...
  \begin{enumerate}[wide, noitemsep, label=(Step~\arabic*).]
    ...
    \item Eliminate all but one independent variable, so that the objective variable becomes a single-variable function in the remaining independent variable. Also figure out the domain of the resulting function using the given scenario.
...
Example~\ref{ex:optimization-swim} is a slight variation of Example~\ref{ex:optimization-fence}. A first-attempt description of a given problem results in hidden information. Develop algebra and geometry skills to uncover potentially hidden information (mathematical relations). Hint: If you have \(n\) independent variables, then you should have at least \(n\) equations in Step~3.

\begin{example} \label{ex:optimization-swim}
  A  river is \(1\) km wide. A person is standing on the bank of the river and wants to reach the opposite side, \(2\) km down the river. \updatenote{"want" corrected to "wants".} The person can swim \(3\) km/hr and walk \(6\) km/hr. 
  ...
  \begin{enumerate}[wide, noitemsep, label=(Step~\arabic*).]
    \item The sketch is already given. \emph{Introduce variables} to measure relevant quantities.
...
\begin{exercise} \label{ex:optimization-ladder}
  ...
  \begin{enumerate}[wide, noitemsep, label=(Step~\arabic*).]
    ...
    \item \hlattn{Achieve the goal we wrote down in Step (2)}. Be very clear of the mathematical problem we are about to solve.
      ...
      Hint: The functions \(f(x)\) and \(f(x)^{2}\) share the \(x\)-value at which their absolute minimum occur.  This does \emph{not} imply that \(f(x)\) and \(f(x)^{2}\) have the same absolute minimum.  If you can find the \(x\)-value, call it \(c\), at which \(f(x)^{2}\) reaches its absolute minimum, then \(f(c)\) is the absolute minimum for \(f\).  This trick only works for minima.
...
\begin{exercise}
  Take a standard printer paper which has dimension \(8.5 \times 11\) inches. Fold it in half to create a prism. What is the angle between the two folded sides that maximizes the volume of the resulting prism?
...
% \begin{exercise} \label{ex:optimization-closest-point-to-curve}
