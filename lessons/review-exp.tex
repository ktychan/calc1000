...
\begin{exercise} \label{ex:exp-change-exercise-2}
  ...
  \begin{enumerate}[wide, label={Method (\arabic*).}]
    \item Combine the above change-base-to-\(e\) and l'H\^opital's rule.  This methods is widely applicable but requires some algebra steps.

      \blanklines{10}

    \item Perform a change of variable \(u = 2x\) and use the property of continuous functions.  This methods is fast but requires us to  \hlmain{recognize} \(e\) from a certain limit stated on page~\ref{def:exp-ln}.
...
Some exercises targetting the use of identities for exponential and logarithmic functions. 
\begin{exercise}
