...
\faStar{} \hlmain{Change base to \(e\)} means apply Equation~\eqref{eq:exp-change-of-base} and is a \hlmain{powerful universal method} to deal with \enquote{weird} exponential-like functions, such as \(x^{x}\) or \((1+x)^{\sin(x)}\). This technique will be relevant in evaluating limits and derivatives of such functions.
