...
A function \(b^{x}\), where \(b\) is a constant so that \(b > 0\) and \(b \ne 1\), is called an \hlmain{exponential function with base \(b\)} and exponent \(x\). \updatenote{Added the missing comma closing the "where..." clause.} Within the context of our textbook, every exponential function \(b^{x}\) has an \hlsupp{inverse} called the \hlmain{logarithmic function with base \(b\)}, denoted by \(\log_{b}(x)\). \newline
