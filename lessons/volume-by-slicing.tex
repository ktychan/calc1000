...
\begin{thm}
  Let \(S\) be a solid (in three dimensions) that lies over an interval \([a,b]\), then
  \begin{equation} \label{eq:volume}
    (\text{volume of } S) = \int_{a}^{b} A(x) \;dx,
  \end{equation}
  where \(A(x)\) is the cross-sectional area of a particular slice. \updatenote{"three dimension" corrected to "three dimensions".}
...
The above idea applies particularly nicely (after some practice) to \emph{solids of revolution} which are obtained by rotating a region in a plane about a line in space. \updatenote{Subject-verb agreement: "which is obtained" corrected to "which are obtained" (plural "solids").} \\
