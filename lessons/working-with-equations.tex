...
The wording pattern of a type of questions we frequently encounter is 
...
\begin{itemize}
  \item Find values for the spring constant for which a spring satisfying Hooke's Law stretches by \(3\) centimetres under a force of \(5\) Newtons?
...
\begin{example} \label{eg:find-all-blah-continuity}
  ...
  A variation: Remember to draw a conclusion based on relevant equations.
  Find all unknown constants \(b\) for which \(f(x) = \begin{cases} 2x + b &\text{if } x < 1 \\ 1 &\text{if } x = 1 \\ \frac{\sqrt{x} - 1}{x - 1} & \text{if } x > 1 \end{cases}\) is continuous everywhere. 

  \begin{tikzpicture}[scale=1]
    \node[rotate={pi}, inner sep=0pt] {\parbox{\linewidth}{\scriptsize Solution: Such a number \(b\) does not exist because \(\lim_{x \to 1^{+}}f(x) \ne f(1)\).}};
  \end{tikzpicture}
\end{example}
...
For differentiability, Equations~\eqref{eq:def-derivative-1} and \eqref{eq:def-derivative-2} give us equations for differentiability.  
\[
