...
\begin{definition} \label{def:definite-integral}
  ...
  \begin{equation} \label{eq:definite-integral} 
    \int_{\alpha}^{\beta} f(x) \;dx = \lim_{n \to \infty} \sum_{i=1}^{n} f(x_{i}^{*}) \Delta x.
  \end{equation}
...
\end{definition}
\faStar You are strongly encouraged to notice and remember that Definitions~\ref{def:riemann-sum}~and~\ref{def:definite-integral} are two sides of the same coin!
\[
...
\begin{align*}
  \int_{a}^{b} (\text{constant}) f(x) \;dx &= (\text{constant}) \int_{a}^{b} f(x) \;dx && \text{(constant multiple)}\\
...
The split/merge property is \hlmain{necessary} for evaluating definite integrals of piecewise functions.
\begin{example}
