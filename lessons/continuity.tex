...
Functions that model nature typically don't ``break.'' Conversely, ``breaks'' in functions are really horrible to work with. We make such a notion precise. \updatenote{Added the missing "a".}

\begin{definition}[continuity] \label{def:continuity}
  A function \(f(x)\) is \emph{continuous at a number \(a\)} if 
  \[
    {\lim_{x \to a} f(x) = f(a).}
  \]
  \textbook{Page 180}
...
\begin{figure}[h]  % [h] for here, [ht] for here top, [hb] for here bottom
  \centering
  \includestandalone{../standalones/plot_discontinuity_examples}
...
A function is called \hlmain{continuous over an interval} if it is continuous at every number in that interval. At the endpoints of each interval, we only require a function to be continuous from one side. \updatenote{"one-side" corrected to "one side".}
...
Continuity and limits are closely related as we can see from the definition of continuity: \updatenote{"Continuities" corrected to "Continuity".} Whenever a function \(f(x)\) is {continuous} at \(a\), then we can evaluate \(\lim_{x \to a} f(x)\) by direct substitution. 
...
\begin{example} \label{eg:continuity-and-cancellation}
...
\begin{exercise}[Midterm, Fall 2024]
  ...
  \[
    ...
    \begin{cases}
      (x-1)(x+2), &\text{ x < -1 } \\
      \frac{x-1}{x^{2}+1}, &\text{ -1 \le x < 1 } \\
      1, &\text{x = 1}\\
      \frac{x-1}{x^{2}+5}, &\text{x > 1}.
    \end{cases}
  \]
