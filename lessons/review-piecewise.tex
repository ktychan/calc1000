...
\begin{document} \section{Review of piecewise functions and absolute values} \label{sec:review-piecewise}
...
%   \faExclamationTriangle{} One-sided limits are \hlmain{universal fail-safe tools} to solve calculus problems (evaluate \hlsupp{limits}, classify \hlsupp{discontinuities}, calculate \hlsupp{derivatives}, etc.) \hlwarn{at endpoints of subdomains} of piecewise functions. \updatenote{Added the missing period in "etc.".}  The specific expression in the limit is \emph{not always} just \(f(x)\) and depends on the problem.
% \end{mdframed}
...
\begin{exercise} \label{ex:piecewise-abs-of-function}
  Write down \(|(x + 2)(x - 4)|\) as using the left-brace notation for piecewise functions.
...
\begin{exercise}
  Write down \(\left| \frac{x + 2}{x - 4} \right|\) as using the left-brace notation for piecewise functions.
