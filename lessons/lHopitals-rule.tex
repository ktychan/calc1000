...
\begin{document} \section{L'H\^opital's rule}
  ...
  The method to evaluate indeterminate forms of type \(0^{0}, \infty^{0}, 1^{\infty}\) \hlsupp{borrows the idea} from logarithmic differentiation. Recall that the \emph{universal} method to deal with functions \(f(x)^{g(x)}\) is to change base to \(e\).
  \blanklines{10}
  \begin{example}
