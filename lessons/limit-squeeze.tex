...
This exercise is very similar to Example~\ref{ex:limit-law-x-square-sin(x)}. 

\begin{exercise} \label{ex:limit-law-x-sin(x)}
  Evaluate \(\lim_{x \to 0} \left|x \sin\left(\frac{1}{x}\right)\right|\).

  {\footnotesize Hint: \(|ab| = |a|\cdot|b|\) is true for all real numbers \(a,b\).}
  
  \blanks*
\end{exercise}

The next exercises are direct application of the squeeze theorem. No need to choose an upper and a lower bound for \(g(x)\) because they are already given.  It is more of a ``do I remember the statement of the squeeze theorem?'' type of exercise.
...
This requires a combination of the techniques from Exercise~\ref{ex:limit-law-x-sin(x)} and \ref{ex:limit-squeeze-easy}.
...
%
%   \item The piecewise nature of \(f(x)\) and \(h(x)\) forces us to show \(f(x) \le x \sin(1/x) \le h(x)\) in two steps.
%     \begin{enumerate}
